...
impl<'a> Generator<'a> {
    ...
    fn write_block_def(&mut self, ws1: &WS, name: &str, nodes: &'a [Node],
        ...
        self.writeln(&format!(
            "fn render_block_{}_to(&self, writer: &mut ::std::fmt::Write) -> Result<(), ::std::fmt::Error> {{",
            name));
        self.prepare_ws(ws1);
        self.handle(nodes);
        self.flush_ws(ws2);
    ...
    fn impl_template(&mut self, ast: &syn::DeriveInput, nodes: &'a [Node]) {
        ...
        self.writeln("fn render_to(&self, writer: &mut ::std::fmt::Write) -> Result<(), ::std::fmt::Error> {");
        self.handle(nodes);
    ...
    fn impl_trait(&mut self, ast: &syn::DeriveInput, trait_name: &str,
        ...
        self.writeln("#[allow(unused_variables)]");
        self.writeln(&format!(
            "fn render_trait_to(&self, timpl: &{}, writer: &mut ::std::fmt::Write) -> Result<(), ::std::fmt::Error> {{",
            trait_name));
